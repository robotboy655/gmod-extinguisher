...
function SWEP:DoExtinguish( pushforce, effectscale )
	...
	if ( CLIENT ) then
		if ( owner == LocalPlayer() ) then self:DoEffect( effectscale ) end -- FIXME
		return
	end
	...
	for id, prop in pairs( ents.FindInSphere( pos, 80 ) ) do
		...
		if ( pushforce > 0 ) then
			...
			if ( IsValid( physobj ) ) then
				physobj:ApplyForceOffset( owner:GetAimVector() * pushforce, pos )
			end
		end
...
function SWEP:PrimaryAttack()
	...
	if ( IsFirstTimePredicted() ) then
		...
		if ( SERVER ) then
			...
			local owner = self:GetOwner()
			if ( owner:KeyPressed( IN_ATTACK ) or !self.Sound ) then
				self:SendWeaponAnim( ACT_VM_PRIMARYATTACK )

				self.Sound = CreateSound( owner, Sound( "weapons/extinguisher/fire1.wav" ) )
...
function SWEP:Think()
	...
	local owner = self:GetOwner()
	if ( ( self.NextAmmoReplenish or 0 ) < CurTime() and owner:WaterLevel() > 1 ) then
		if ( !self.IsInfinite and self:Ammo1() < self.MaxAmmo * 2 ) then
			owner:SetAmmo( math.min( self:Ammo1() + 25, self.MaxAmmo * 2 ), self:GetPrimaryAmmoType() )
		end
	...
	if ( owner:KeyReleased( IN_ATTACK ) or ( !owner:KeyDown( IN_ATTACK ) and self.Sound ) ) then
...
function SWEP:DoIdleAnimation()
	...
	if ( owner:KeyDown( IN_ATTACK ) and self:Ammo1() > 0 ) then self:SendWeaponAnim( ACT_VM_IDLE_1 ) return end
	if ( owner:KeyDown( IN_ATTACK ) and self:Ammo1() < 1 ) then self:SendWeaponAnim( ACT_VM_IDLE_EMPTY ) return end
...
function SWEP:GetAnimationTime()
	...
	if ( time == 0 and IsValid( owner ) and !owner:IsNPC() and IsValid( owner:GetViewModel() ) ) then time = owner:GetViewModel():SequenceDuration() end
	return time
end
